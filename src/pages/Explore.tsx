...
import { Sparkles, Search, Filter, Compass, Heart, MessageCircle, User, TrendingUp, Flame } from "lucide-react";
    ...
    <div className="min-h-screen pb-20">
      ...
      <header className="sticky top-0 z-40 glass border-b border-border/20 px-4 py-3 fade-in">
        <div className="flex items-center gap-3 max-w-2xl mx-auto">
          ...
          <h1 className="text-xl font-display text-gradient-aurora flex-1">Explore</h1>
          {user ? (
            <Link to="/dashboard">
              <Button variant="ghost" size="sm" className="text-muted-foreground text-xs">Home</Button>
            </Link>
          ) : (
            <Link to="/auth">
              <Button size="sm" className="bg-primary hover:bg-primary/90 text-xs">Join</Button>
            </Link>
          )}
        </div>
      </header>
      ...
      <div className="max-w-2xl mx-auto px-4">
        {/* Stories */}
        <div className="py-4 border-b border-border/10 fade-in" style={{ animationDelay: "0.05s" }}>
          <StoriesBar />
        </div>

        {/* Tabs */}
        <Tabs defaultValue="feed" className="mt-4 fade-in" style={{ animationDelay: "0.1s" }}>
          <TabsList className="glass w-full justify-start gap-0 overflow-x-auto">
            <TabsTrigger value="feed" className="text-xs">Feed</TabsTrigger>
            <TabsTrigger value="trending" className="text-xs">
              <Flame className="w-3 h-3 mr-1" />Trending
            </TabsTrigger>
            {user && <TabsTrigger value="activity" className="text-xs">Activity</TabsTrigger>}
          ...
          <TabsContent value="feed" className="mt-4">
            ...
            <div className="glass rounded-xl p-3 mb-4">
              <div className="flex gap-2">
                ...
                <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)}
                  className="bg-muted/50 border border-border/50 rounded-lg px-2 text-xs text-foreground focus:border-primary outline-none">
                  {categories.map((cat) => (
            ...
            {filteredDreams.length === 0 ? (
              <div className="glass-card p-10 text-center">
                <Compass className="w-12 h-12 text-primary/50 mx-auto mb-3" />
                <h3 className="font-display text-lg mb-2">No dreams found</h3>
          ...
          <TabsContent value="trending" className="mt-4">
            {trendingDreams.length === 0 ? (
              <div className="glass-card p-10 text-center">
                ...
                <h3 className="font-display text-lg mb-2">No trending dreams</h3>
              ...
              <div className="space-y-4">
                {trendingDreams.map((dream, i) => (
                  <div key={dream.id} className="relative">
                    {i < 3 && (
                      <div className="absolute -left-1 -top-1 z-10 w-6 h-6 rounded-full bg-accent flex items-center justify-center text-[10px] font-bold text-accent-foreground">
                        #{i + 1}
                      </div>
                    )}
          ...
          {user && (
            <TabsContent value="activity" className="mt-4">
              <div className="glass-card p-4">
                <h2 className="font-display text-lg text-foreground mb-3">Following Activity</h2>
                <ActivityFeed />
              </div>
            </TabsContent>
          )}
          ...
          <TabsContent value="people" className="mt-4">
            <div className="glass-card p-4">
              <h2 className="font-display text-lg text-foreground mb-3">Find Dreamers</h2>
              <UserSearch />
            </div>
          </TabsContent>
        </Tabs>
